...
void ALMBPlayerController::BeginPlay()
{
    ...
    // Enhanced Input Mapping 등록
    if (ULocalPlayer* LP = GetLocalPlayer())
    {
        if (UEnhancedInputLocalPlayerSubsystem* Subsystem = LP->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
        {
            if (IMC_LMBPlayerInput)
            {
...
void ALMBPlayerController::SetupInputComponent()
{
    ...
    if (UEnhancedInputComponent* EIC = Cast<UEnhancedInputComponent>(InputComponent))
    {
        ...
        if (IA_Move1P)
        {
            EIC->BindAction(IA_Move1P, ETriggerEvent::Triggered, this, &ALMBPlayerController::OnInputMove1P);
            EIC->BindAction(IA_Move1P, ETriggerEvent::Completed, this, &ALMBPlayerController::OnInputMove1P);
        }
        if (IA_Move2P)
        {
            EIC->BindAction(IA_Move2P, ETriggerEvent::Triggered, this, &ALMBPlayerController::OnInputMove2P);
            EIC->BindAction(IA_Move2P, ETriggerEvent::Completed, this, &ALMBPlayerController::OnInputMove2P);
        }

        // 1P 스킬
        if (IA_Skill1P_U) EIC->BindAction(IA_Skill1P_U, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill1P_U);
        if (IA_Skill1P_I) EIC->BindAction(IA_Skill1P_I, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill1P_I);
        if (IA_Skill1P_J) EIC->BindAction(IA_Skill1P_J, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill1P_J);
        if (IA_Skill1P_K) EIC->BindAction(IA_Skill1P_K, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill1P_K);

        // 2P 스킬
        if (IA_Skill2P_4) EIC->BindAction(IA_Skill2P_4, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill2P_4);
        if (IA_Skill2P_5) EIC->BindAction(IA_Skill2P_5, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill2P_5);
        if (IA_Skill2P_1) EIC->BindAction(IA_Skill2P_1, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill2P_1);
        if (IA_Skill2P_2) EIC->BindAction(IA_Skill2P_2, ETriggerEvent::Started, this, &ALMBPlayerController::OnSkill2P_2);
    }
}
...
void ALMBPlayerController::AddPawnPlayer(ALMBpawnPlayer* NewPlayer)
{
    ...
    if (!PawnPlayers.Contains(NewPlayer))
    {
        ...
        UE_LOG(LogTemp, Warning, TEXT("PawnPlayers에 %d번 플레이어 추가"), NewPlayer->GetPlayerIndex());
    }
    else
    {
...
// 안전하게 nullptr 체크 후 입력 처리
void ALMBPlayerController::OnInputMove1P(const FInputActionValue& Value)
{
    if (PawnPlayers.IsValidIndex(0) && PawnPlayers[0])
        PawnPlayers[0]->OnInputMove(Value.Get<FVector2D>());
}

void ALMBPlayerController::OnInputMove2P(const FInputActionValue& Value)
{
    if (PawnPlayers.IsValidIndex(1) && PawnPlayers[1])
        PawnPlayers[1]->OnInputMove(Value.Get<FVector2D>());
}

// 1P 스킬
void ALMBPlayerController::OnSkill1P_U() { if (PawnPlayers.IsValidIndex(0) && PawnPlayers[0]) PawnPlayers[0]->UseSkill(0); }
void ALMBPlayerController::OnSkill1P_I() { if (PawnPlayers.IsValidIndex(0) && PawnPlayers[0]) PawnPlayers[0]->UseSkill(1); }
void ALMBPlayerController::OnSkill1P_J() { if (PawnPlayers.IsValidIndex(0) && PawnPlayers[0]) PawnPlayers[0]->UseSkill(2); }
void ALMBPlayerController::OnSkill1P_K() { if (PawnPlayers.IsValidIndex(0) && PawnPlayers[0]) PawnPlayers[0]->UseSkill(3); }

// 2P 스킬
void ALMBPlayerController::OnSkill2P_4() { if (PawnPlayers.IsValidIndex(1) && PawnPlayers[1]) PawnPlayers[1]->UseSkill(0); }
void ALMBPlayerController::OnSkill2P_5() { if (PawnPlayers.IsValidIndex(1) && PawnPlayers[1]) PawnPlayers[1]->UseSkill(1); }
void ALMBPlayerController::OnSkill2P_1() { if (PawnPlayers.IsValidIndex(1) && PawnPlayers[1]) PawnPlayers[1]->UseSkill(2); }
void ALMBPlayerController::OnSkill2P_2() { if (PawnPlayers.IsValidIndex(1) && PawnPlayers[1]) PawnPlayers[1]->UseSkill(3); }